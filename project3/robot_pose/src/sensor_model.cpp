...
void SensorModel::computePointCloud(sensor_msgs::PointCloud &cloud, Pose &pose, sensor_msgs::LaserScan *scan) {
	...
	for(int i = 0; cur_angle <= angle_max; i = i + interval, cur_angle = cur_angle + interval * angle_increment) {
...
bool SensorModel::computeWeight(Pose &pose, sensor_msgs::LaserScan *scan) {
	...
	try {
		const sensor_msgs::PointCloud in = cloud;
		transformListener->transformPointCloud("/map", in, mapCloud);
	} catch (tf::TransformException &ex) {
		ROS_ERROR("Could not tranform point cloud to map frame: %s\n", ex.what());
		return false;
	}	
	pose.weight = 1.0;
